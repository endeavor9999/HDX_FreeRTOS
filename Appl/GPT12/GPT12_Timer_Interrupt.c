...
#define RELOAD_VALUE                98u                  /* Reload value to have an interrupt each 500ms         */
...
{
    Ifx_TickTime g_ticksFor1us;

    g_ticksFor1us=IfxStm_getTicksFromMicroseconds(BSP_DEFAULT_TIMER, WAIT_TIME);
    waitTime(us * g_ticksFor1us); //10MS
}

void delay_ns(uint32 ns)
{
    Ifx_TickTime g_ticksFor1ns;

   // g_ticksFor1ns=IfxStm_getTicksFromNanoseconds(BSP_DEFAULT_TIMER, WAIT_TIME);
    waitTime(ns * g_ticksFor1ns); //10MS
}
...
void TC1_Timer_Wait_1mS(Timer_Configuration *Timer,uint32 Time)
{
    if(Timer->Flag==Wait_Standby)
    {
        Timer->Time_Start=Return_Timer_Value();
        Timer->Flag=Wait_Runnig;
    }

    else if(Timer->Flag==Wait_Runnig)
    {
        Timer->Timer_Check=Return_Timer_Value();
        if(Timer->Timer_Check>=Timer->Time_Start+Time)
        {
            Timer->Flag=2;
        }
    }
